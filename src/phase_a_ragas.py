...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    for a, pq in zip(answers, per_q):
        f = getattr(pq, "faithfulness", 0.0)
        ar = getattr(pq, "answer_relevancy", 0.0)
        cp = getattr(pq, "context_precision", 0.0)
        cr = getattr(pq, "context_recall", 0.0)
...
def bottom_10(results: list[RagasResult]) -> list[dict]:
    ...
    for i, r in enumerate(bottom):
        diag, fix = DIAGNOSTIC_TREE.get(r.worst_metric, ("Unknown error", "Check pipeline logs"))
        output.append({
            "rank": i + 1,
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    if not results:
        return {
            "matrix": matrix,
            "dominant_failure_distribution": "factual",
            "dominant_failure_metric": "faithfulness",
            "insight": "No results to analyze.",
        }

    for r in results:
        w_metric = r.worst_metric
        dist = r.distribution
        if w_metric in matrix and dist in matrix[w_metric]:
            matrix[w_metric][dist] += 1

    dist_counts = {
        d: sum(matrix[m].get(d, 0) for m in matrix)
    ...
    insight = (
        f"Distribution '{dominant_dist}' có nhiều failure nhất ({dist_counts[dominant_dist]} câu). "
        f"Metric '{dominant_metric}' là điểm yếu chủ đạo ({sum(matrix[dominant_metric].values())} câu). "
